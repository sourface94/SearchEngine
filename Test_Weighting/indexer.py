...
headings = {} #contains the titles ad headings
...
def get_headings(page_contents):
	headingsandtitles = re.sub(r'<title\b[^<]*(?:(?!<\/title>)<[^<]*)*<\/title>', '', page_contents.strip())
	headingsandtitles = re.sub(r'<h1\b[^<]*(?:(?!<\/h1>)<[^<]*)*<\/h1>', '', headingsandtitles)
	headingsandtitles = re.sub(r'<h2\b[^<]*(?:(?!<\/h2>)<[^<]*)*<\/h2>', '', headingsandtitles)
	headingsandtitles = re.sub(r'<h3\b[^<]*(?:(?!<\/h3)<[^<]*)*<\/h3>', '', headingsandtitles)
	headingsandtitles = re.sub(r'<h4\b[^<]*(?:(?!<\/h4>)<[^<]*)*<\/h4>', '', headingsandtitles)
	headingsandtitles = headingsandtitles.strip
	return headingsandtitles
...
def make_index(url, page_contents):
	...
	headingsandtitles = get_headings(page_contents)
	
	for term in headingsandtitles:
		term = term.lower()
		if term in vocab:
